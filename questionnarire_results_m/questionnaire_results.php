...
//TeacheCd選択ボタン
$stmt = $dbh->prepare('select teachercd from t_teacher group by teachercd');
$stmt->execute();
$testA = $stmt->fetchAll();
$i = 0;
foreach($testA as $row){
  $teachercdSelect[$i] = $row['teachercd'];
  $i++;
}

$teachercd = $teachercdSelect[0];//先頭のteachecdをデフォルトとして使用
...
//select semester from t_questimplement where year = :year order by semester desc
//修正!!
...
<html>
    <head>
        ...
        <script type="text/javascript" src="internalProcessing.js"></script>
    ...
    <body link="blue" vlink="blue">
        <!-- サイドメニュー -->
        <?php include '../include_file/master_side_menu.php'; ?>

        
        <!-- イベント通知 -->
        <div class="event_notice">
            <center><font size="10px">アンケート参照：集計</center></font>
        </div>
        ...
        <div class="main">
            ...
            <div id="select_box">
                <fieldset>
                    ...
                    <div style="margin-left: auto; margin-right:auto;width:550px;margin-top:25px">

			<div class="select_h" style="margin-top: -35px; margin-left: -200px">教員
                            <select name="teachercd" id="select0">
                                <?php
                                foreach($testA as $row) {
                                echo "<option value=\"{$row['teachercd']}\">{$row['teachercd']}</option>";
                                }
                              ?>
                            </select>
                        </div>

                        <div class="select_h"style="margin-top: -35px; margin-left: 10px">年度
